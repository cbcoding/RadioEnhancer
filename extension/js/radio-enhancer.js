...
var playerControl = function(action) {
    ...
    case "mute":
        ...
        chrome.extension.sendRequest({
            ...
            jQuery('.volumeKnob').simulate("drag", {
                dx: -150,
                dy: 0
            });
...
var hideAds = function() {
    ...
    jQuery("#mainContainer").css({
        "background-image": settings.background_image + " !important",
...
var selectableLyrics = function() {
    ...
    if(jQuery("#RE-copyLyrics").length == 0) {
        jQuery(".item.lyrics > .heading").append('<span id="RE-copyLyrics"> - Copy Lyrics to Clipboard</span>').css({
            cursor: "pointer"
        });
    }
...
var doSongChange = function() {
    ...
    if(currentAlbumArt == null || oldAlbumArt == newAlbumArt) {
        if(song_skip_tries < 20) //album art fix
        {
            ...
            setTimeout(function() {
                doSongChange();
            }, 100); //try again in 1/10 of second.
            return;
        }
    }
...
var showNewSongPopup = function() {
    ...
    if(songName == "audioad")
    {
        debugLog("RadioEnhancer - Enabling Pandora UI Controls");
        jQuery("#playerBar > .highlight > .disabled").remove();
        jQuery("#playerBar > .highlight > .columns > .leftcolumn, #playerBar > .highlight > .columns > .middlecolumn").css("opacity", "1");
...
*/
                ...
                //modal window
                +
                '<div id="RE-modal" style="' + 'z-index:9999 !important;' + 'height:auto;' + 'width:420px;' + 'color:#000000;' + 'background:-webkit-linear-gradient(top, #ffffff 0%,#e5e5e5 100%);' + 'border:2px solid #d0d0d0;' + '-webkit-border-radius: 8px;' + '-webkit-box-shadow: 0 0 50px #000;' + 'text-align: center;' + '">'
