...
var sendAPIRequest = function(requestType, requestData, requestMethod, callbackFxn) {
		...
		if(requestType == 'track.scrobble') //we don't want to scrobble twice in a row.
		{
			...
			if(localStorage['last_scrobble_artist'] && requestParams['artist'] == localStorage['last_scrobble_artist'] && localStorage['last_scrobble_track'] && requestParams['track'] == localStorage['last_scrobble_track']) {
				return;
			}
